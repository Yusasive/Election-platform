...
export default function VotingPage() {
  ...
  const [electionSettings, setElectionSettings] =
    useState<ElectionSettings | null>(null);
  ...
  const { mutate: submitVote, loading: submittingVote } =
    useApiMutation("/votes");
  ...
  const showTimingToast = useCallback(
    (message: string, type: "warning" | "info" = "warning") => {
      const now = Date.now();
  ...
  useEffect(() => {
    if (
      ...
      const retryTimer = setTimeout(() => {
        ...
        toast(
          `Retrying to load candidates... (${retryCount + 1}/3)`,
          { icon: '🔄' }
        );
        refetchCandidates();
      }, retryDelay);
  ...
  useEffect(() => {
    ...
    try {
      ...
      toast.success(
        `Welcome ${parsedVoterData.fullName}! Loading your ballot...`,
        { duration: 3000 }
      );
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const checkVotingStatus = () => {
      ...
      if (loginTimeLeft <= 5 * 60 * 1000 && loginTimeLeft > 0) {
        // 5 minutes left
        showTimingToast(
          `⚠️ Login session expires in ${formatTime(loginTimeLeft)}!`,
          "warning"
        );
      }

      if (votingTimeLeft <= 10 * 60 * 1000 && votingTimeLeft > 0) {
        // 10 minutes left
        showTimingToast(
          `⏰ Voting ends in ${formatTime(votingTimeLeft)}!`,
          "warning"
        );
      }
    };
  ...
  const handleSelection = (
    ...
    if (allowMultiple) {
      ...
      if (updatedSelections.includes(candidateId)) {
        ...
        toast(`Deselected candidate for ${position}`, { 
          duration: 2000, 
          icon: '↩️' 
        });
      }
  ...
  const handleVote = async () => {
    ...
    if (missingVotes.length > 0) {
      toast.error(`Please vote for all positions: ${missingVotes.join(", ")}`, {
        duration: 5000,
      });
      return;
    }
    ...
    try {
      ...
      toast.success("Vote submitted successfully! Redirecting...", {
        id: loadingToast,
      });
  ...
  useEffect(() => {
    if (candidatesError && retryCount >= 3) {
      toast.error(
        "Failed to load candidates after multiple attempts. Please refresh the page.",
        { duration: 8000 }
      );
    }
    if (settingsError) {
      toast.error(
        "Failed to load election settings. Please refresh the page.",
        { duration: 6000 }
      );
    }
      ...
      <main className="flex justify-center items-center h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50">
        ...
        >
          ...
          <p className="text-sm text-gray-500 mt-2">
            Please wait while we prepare your ballot
          </p>
      ...
      <main className="flex justify-center items-center h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50">
        ...
        >
          ...
          <p className="text-gray-600 mb-4">
            We couldn&apos;t load the candidate data after multiple attempts.
          </p>
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {position.candidates.map((candidate: Candidate) => (
                  ...
                  >
                    <div
                      className={`relative p-6 rounded-2xl border-2 transition duration-300 ${
                        (position.allowMultiple
                    ...
                    >
                      ...
                      <div className="absolute top-4 right-4">
                        {position.allowMultiple ? (
                          <div
                            className={`w-6 h-6 rounded border-2 flex items-center justify-center transition duration-200 ${
                              selections[position.position]?.includes(
                                candidate.id.toString()
                              )
                      ...
                      <div className="text-center space-y-3">
                        ...
                        {candidate.nickname && (
                          <p className="text-sm text-blue-500 font-semibold italic tracking-wide">
                            &quot;{candidate.nickname}&quot;
                          </p>
                        )}
        ...
        <div className="mt-6 flex justify-center space-x-4 text-sm text-gray-500">
          <button
            onClick={() => {
              ...
              Promise.all([refetchSettings(), refetchCandidates()])
                .then(() => {
                  toast.dismiss();
          ...
          <button
            onClick={() => {
              ...
              toast(
                `Progress: ${selectionCount}/${totalPositions} positions voted`,
                { duration: 3000, icon: "📊" }
              );
            }}
            className="hover:text-blue-500 transition duration-200"
